...
/* ---------- EXCEL EXPORT (requires XLSX global from SheetJS CDN) ---------- */
function exportRowsToExcel(rows, columns, filename, sheetName){
  ...
  if(typeof XLSX === 'undefined'){
    showToast('Excel export library did not load — check your connection.', 'error');
    return;
  }
  const data = rows.map(row => {
    const obj = {};
    columns.forEach(c => { obj[c.label] = row[c.key] !== null && row[c.key] !== undefined ? row[c.key] : ''; });
    return obj;
  });
